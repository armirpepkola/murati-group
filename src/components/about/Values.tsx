    ...
    <section className="py-32 px-6">
      <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-16 items-center">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <h2 className="text-4xl font-bold text-[#4a5261] mb-12">
            Built On These Principles
          </h2>

          <div className="relative border-l-4 border-[#e25858] pl-6 space-y-14">
            {values.map(({ title, description }, i) => (
              ...
              >
                <CheckCircle className="text-[#e25858] min-w-[24px] mt-1" size={24} />
                <div>
                  <h3 className="text-xl font-semibold text-[#4a5261]">{title}</h3>
                  <p className="text-gray-700 mt-1 leading-relaxed">{description}</p>
                </div>
              </motion.div>
            ))}
          </div>
